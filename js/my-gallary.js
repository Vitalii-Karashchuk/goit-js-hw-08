...
const collection = images.map((elem) => {
    const listItem = document.createElement('li');
    listItem.classList.add('gallery-item');
    const linkItem = document.createElement('a');
    linkItem.classList.add('gallery-link'); 
    linkItem.setAttribute('href', elem.original);
    const image = document.createElement('img');
    image.classList.add('gallery-image');
    image.setAttribute('src', elem.preview);
    image.dataset.source = elem.original;
    image.setAttribute('alt', elem.description);
    
    listItem.append(linkItem, image);
        return listItem;
});
...
console.log("container:", container);
  ...
  const handleImg = (event) => {
    ...
    const instanse = basicLightbox.create(`
       <div class="modal">
            <img src=`${currentPikche.img}` alt=`${currentPikche.description}`>
        </div>
    `)
    instanse.show()
 };
